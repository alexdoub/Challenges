...
object CombinationSumII {
    ...
    fun combinationSum2(candidates: IntArray, target: Int): List<List<Int>> {
        ...
        fun buildSums(index: Int, rem: Int, used: ArrayList<Int>) {
            ...
            for (x in index + 1..candidates.lastIndex) {

                // stop checking next rolls if we're already over the limit
                if (rem - candidates[x] < 0) break

                //skip repeat numbers -- if its the same as the last branch then don't try it (only branch on UNIQUE numbers, as per instructions)
                if (x != index + 1 && candidates[x] == candidates[x - 1]) continue

                used.add(candidates[x])
                buildSums(x, rem - candidates[x], used)
                used.removeAt(used.lastIndex)
            }
        }
        buildSums(-1, target, ArrayList<Int>())
        return sums
    }
