...
object BinaryTreeMaxPathSum3 {
    ...
    fun maxPathSumWithBranching(root: TreeNode?): Int {
        ...
        val notIncludingRoot = Math.max(
            maxPathSumWithBranching(root.left),
            maxPathSumWithBranching(root.right)
        )
        val includingRoot = root.`val` + maxPathSumWithoutBranching(
            root.left,
            0
        ) + maxPathSumWithoutBranching(root.right, 0)
